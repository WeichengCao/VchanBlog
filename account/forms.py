...
class RegistrationForm(Form):
    ...
    password = PasswordField("Password", validators=[Required(), Length(1, 128), EqualTo('password2', message='Passwords must match')])
    password2 = PasswordField("Comfirm password", validators=[Required()])

    def validate_username(self, field):
        if models.CUser.objects.filter(username=field.data).count() > 0:
            raise ValidationError("Username already in use")

    def validate_email(self, field):
        if models.CUser.objects.filter(email=field.data).count() > 0:
...
class SuUserForm(Form):
    ...
    display_name = StringField("Display Name", validators=[Length(1,128)])
    biography = StringField("Biography")
    homepage_url = SelectField("Homepage", validators=[URL(), Optional()])
    weibo = StringField("Weibo", validators=[URL(), Optional()])
    weixin = StringField("Weixin", validators=[URL(), Optional()])
    twitter = StringField("Twitter", validators=[URL(), Optional()])
    github = StringField("github", validators=[URL(), Optional()])
    facebook = StringField("Facebook", validators=[URL(), Optional()])
    weibo = StringField("Linkedin", validators=[URL(), Optional()])
